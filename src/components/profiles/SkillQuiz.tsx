        ...
        {!selectedQuizKey ? (
          ...
          <div className="space-y-6">
                ...
                return (
                  <button
                    ...
                    className={`w-full p-4 rounded-xl border text-left text-sm transition-all flex items-center justify-between ${
                      isSelected
                        ? 'bg-emerald-500/10 border-emerald-500 text-white font-semibold shadow-md shadow-emerald-500/5'
                        : 'bg-zinc-900/40 border-zinc-800 text-zinc-300 hover:border-zinc-700 hover:bg-zinc-900/80'
                    }`}
                  >
                    <span>{option}</span>
                    {isSelected && (
                      <span className="w-5 h-5 bg-emerald-500 text-black rounded-full flex items-center justify-center">
            ...
            <div className="flex items-center justify-between pt-4 border-t border-zinc-800/80">
              <button
                onClick={() => setSelectedQuizKey(null)}
          ...
          <div className="text-center py-6 space-y-6">
              ...
              {passed ? (
                <div className="space-y-4">
                  ...
                  <p className="text-sm text-zinc-400 mt-2">
                    Congratulations! You scored <span className="font-bold text-emerald-400">{score}/{activeQuiz.questions.length}</span> ({(score/activeQuiz.questions.length)*100}%). The skill <span className="font-semibold text-white">{activeQuiz.skillName}</span> is now verified on your profile.
                  </p>
                </div>
              ) : (
                <div className="space-y-4">
            ...
            <div className="flex items-center justify-center gap-3 pt-6 border-t border-zinc-800/80">
              <button
                onClick={() => setSelectedQuizKey(null)}
                className="px-5 py-2.5 rounded-xl bg-zinc-900 border border-zinc-800 hover:border-zinc-700 text-xs text-white font-bold transition-all"
              >
                Go Back to Quizzes
              </button>
              {!passed && (
                <button
                  onClick={() => handleStartQuiz(selectedQuizKey)}
                  className="px-5 py-2.5 rounded-xl bg-emerald-500 text-black font-bold text-xs hover:bg-emerald-400 transition-all"
                >
                  Retake Assessment
                </button>
              )}
